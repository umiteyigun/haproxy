...
export class AppConfigurator {
    ...
    ngOnInit() {
        if (isPlatformBrowser(this.platformId)) {
            ...
            // Kayıtlı surface palette'ini geri uygula
            const savedSurface = this.layoutService.layoutConfig().surface;
            if (savedSurface) {
                const palette = this.surfaces.find((s) => s.name === savedSurface)?.palette;
                if (palette) {
